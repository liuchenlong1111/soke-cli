...
func (p *DeviceFlowProvider) Login(ctx context.Context) (*TokenData, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if isInvalidGrantError(err) && attempt < maxAttempts {
			dfPrintWarning(p.output(), "授权码已过期，正在重新发起设备授权流程...")
			_, _ = fmt.Fprintln(p.output(), "")
			continue
		}
		return nil, err
	}
	return nil, fmt.Errorf("%s", "设备授权流程失败（已重试 %d 次）", maxAttempts)
}

func (p *DeviceFlowProvider) loginOnce(ctx context.Context, attempt int) (*TokenData, error) {
	dfPrintStep(p.output(), 1, "请求设备授权码...", attempt)
	_, _ = fmt.Fprintln(p.output(), "")

	authResp, err := p.requestDeviceCode(ctx)
	if err != nil {
		return nil, fmt.Errorf("%s: %w", "请求设备授权码失败", err)
	}
	...
	dfPrintStep(p.output(), 2, "等待用户授权...", 0)
	dfPrintDim(p.output(), fmt.Sprintf("  (每 %d 秒轮询一次)", authResp.Interval))
	_, _ = fmt.Fprintln(p.output(), "")
	...
	_, _ = fmt.Fprintln(p.output(), "")
	dfPrintStep(p.output(), 3, "使用授权码换取 Access Token...", 0)
	...
	dfPrintStep(p.output(), 4, "检查组织 CLI 授权状态...", 0)
	authStatus, authErr := oauthProvider.CheckCLIAuthEnabled(ctx, tokenData.AccessToken)
	if authErr != nil {
		_, _ = fmt.Fprintln(p.output(), "")
		_, _ = fmt.Fprintln(p.output(), dfRed("⚠️  无法检查 CLI 数据访问权限状态"))
		_, _ = fmt.Fprintln(p.output(), "   请检查网络连接后重试。")
		_, _ = fmt.Fprintln(p.output(), "")
		return nil, fmt.Errorf("%s: %w", "检查 CLI 授权状态失败", authErr)
	}
	...
	if denialReason != "" {
		...
		switch denialReason {
		case "user_forbidden":
			_, _ = fmt.Fprintln(p.output(), dfRed("⚠️  该组织已禁止所有成员使用 CLI"))
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("该组织已禁止所有成员使用 CLI")
		case "user_not_allowed":
			_, _ = fmt.Fprintln(p.output(), dfRed("⚠️  您不在该组织的 CLI 授权人员范围内"))
			_, _ = fmt.Fprintln(p.output(), "   请联系组织管理员将您加入 CLI 授权人员名单。")
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("您不在该组织的 CLI 授权人员范围内，请联系组织管理员")
		case "channel_not_allowed":
			ch := os.Getenv("DWS_CHANNEL")
			_, _ = fmt.Fprintf(p.output(), dfRed("⚠️  当前渠道 %s 未获得该组织授权")+"\n", ch)
			_, _ = fmt.Fprintln(p.output(), "   请联系组织管理员开通该渠道的访问权限。")
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, fmt.Errorf("当前渠道 %s 未获得该组织授权，请联系组织管理员", ch)
		case "channel_required":
			_, _ = fmt.Fprintln(p.output(), dfRed("⚠️  当前组织已开启渠道管控"))
			_, _ = fmt.Fprintln(p.output(), "   请升级到最新版本的 CLI 后重试。")
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("当前组织已开启渠道管控，请升级到最新版本的 CLI 后重试")
		case "no_auth":
			_, _ = fmt.Fprintln(p.output(), dfRed("⚠️  认证已失效"))
			_, _ = fmt.Fprintln(p.output(), "   请执行 dws auth 重新登录。")
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("认证已失效，请执行 dws auth 重新登录")
		default:
			// cli_not_enabled or unknown — show existing admin-apply flow
			_, _ = fmt.Fprintln(p.output(), dfRed("⚠️  该组织尚未开启 CLI 数据访问权限"))
			_, _ = fmt.Fprintln(p.output(), "   你所选择的组织管理员尚未开启「允许成员通过 CLI 访问其个人数据」的权限。")
			_, _ = fmt.Fprintln(p.output(), "")
			...
			if adminErr == nil && admins.Success && len(admins.Result) > 0 {
				...
				_, _ = fmt.Fprintf(p.output(), "   %s%s\n", "组织主管理员：", strings.Join(adminNames, "、"))
			}

			_, _ = fmt.Fprintln(p.output(), "   请联系组织主管理员开启后重新登录。")
			_, _ = fmt.Fprintln(p.output(), "")
			_, _ = fmt.Fprintf(p.output(), "   %s%s\n", "管理员操作入口：", config.GetDeveloperSettingsURL())
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("该组织尚未开启 CLI 数据访问权限，请联系管理员开启")
		}
	}
...
func (p *DeviceFlowProvider) requestDeviceCode(ctx context.Context) (*DeviceAuthResponse, error) {
	...
	if err := json.Unmarshal(body, &sr); err != nil {
		return nil, fmt.Errorf("%s: %w", "解析响应失败", err)
	}
	if !sr.Success {
		return nil, fmt.Errorf("%s: [%s] %s", "服务端返回错误", sr.ErrorCode, sr.ErrorMsg)
	}

	var resp DeviceAuthResponse
	if err := json.Unmarshal(sr.Result, &resp); err != nil {
		return nil, fmt.Errorf("%s: %w", "解析设备授权数据失败", err)
	}
	if resp.DeviceCode == "" || resp.UserCode == "" {
		return nil, errors.New("服务端返回了空的 device_code 或 user_code")
	}
...
func (p *DeviceFlowProvider) pollDeviceToken(ctx context.Context, deviceCode string) (*DeviceTokenResponse, error) {
	...
	if err := json.Unmarshal(sr.Result, &resp); err != nil {
		return nil, fmt.Errorf("%s: %w", "解析令牌数据失败", err)
	}
	return &resp, nil
}
...
func (p *DeviceFlowProvider) pollDeviceStatus(ctx context.Context, flowID string) (*DevicePollResponse, error) {
	...
	if !resp.Success && resp.Data.Status == "" {
		return nil, fmt.Errorf("%s: [%s] %s", "服务端返回错误", resp.Code, resp.Message)
	}
	return &resp, nil
}
...
func (p *DeviceFlowProvider) waitForAuthorizationByFlowID(ctx context.Context, auth *DeviceAuthResponse) (*DeviceTokenResponse, error) {
	...
	for {
		...
		if elapsed >= maxPollTotalWait || elapsed >= deadline {
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, fmt.Errorf("%s", "设备授权码已过期（%d 秒），请重试", auth.ExpiresIn)
		}
		...
		if err != nil {
			dfPrintPollResult(p.output(), "network_error", "网络错误，继续重试...")
			if p.logger != nil {
		...
		case StatusApproved:
			dfPrintPollResult(p.output(), "authorized", "授权成功!")
			return &DeviceTokenResponse{AuthCode: pollData.AuthCode}, nil
		case StatusPending:
			dfPrintPollResult(p.output(), "pending", "等待用户授权...")
		case StatusRejected:
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("用户拒绝了授权请求")
		case StatusExpired:
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("设备授权码已过期")
		default:
			dfPrintPollResult(p.output(), "unknown", fmt.Sprintf("未知状态: %s", pollData.Status))
		}
	}
}
...
func (p *DeviceFlowProvider) waitForAuthorizationByDeviceCode(ctx context.Context, auth *DeviceAuthResponse) (*DeviceTokenResponse, error) {
	...
	for {
		...
		case "slow_down":
			...
			dfPrintPollResult(p.output(), "slow_down", fmt.Sprintf("轮询过快，间隔增加至 %ds", int(interval.Seconds())))
		case "access_denied":
			_, _ = fmt.Fprintln(p.output(), "")
			return nil, errors.New("用户拒绝了授权请求")
		case "expired_token":
			...
			return nil, errors.New("设备授权码已过期")
		default:
			dfPrintPollResult(p.output(), "unknown", fmt.Sprintf("未知错误: %s", resp.Error))
		}
	}
}
...
func (p *DeviceFlowProvider) postForm(ctx context.Context, endpoint string, params url.Values) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%s: %w", "创建请求失败", err)
	}
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")

	resp, err := p.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("%s: %w", "发送请求失败", err)
	}
...
func dfPrintDeviceCodeBox(w io.Writer, auth *DeviceAuthResponse) {
	lines := []string{
		"请在浏览器中打开以下链接，并输入授权码：",
		"",
		fmt.Sprintf("  链接: %s", dfBold(auth.VerificationURI)),
		fmt.Sprintf("  授权码: %s", dfBold(dfYellow(auth.UserCode))),
		"",
	}
	...
	lines = append(lines, dfDim(fmt.Sprintf("授权码将在 %d 秒后过期。", auth.ExpiresIn)))
	dfPrintBox(w, lines)
	_, _ = fmt.Fprintln(w, "")
}
...
func dfPrintPollStatus(w io.Writer, count, elapsedSec int) {
	_, _ = fmt.Fprintf(w, "  %s ", dfDim(fmt.Sprintf("[%d] 轮询中... (%ds)", count, elapsedSec)))
}
